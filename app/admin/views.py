# coding:utf8
from . import admin
from flask import render_template, redirect, url_for, flash, session, request,abort  # 消息闪现,消息会话,跳转
from app.admin.forms import LoginForm, TagForm, MovieForm, PreviewForm, PasswordField, PwdForm, AuthForm, RoleForm,AdminForm
...
# 上下文应用处理器,把全局变量封装，展现到模板里面
@admin.context_processor  # 装饰器
def tpl_extra():
    data = dict(
...
def movie_edit(id=None):
    form = MovieForm()  # 实例化一个MovieForm，然后将form传递到前端页面去。
...
def role_add():
    ...
    if form.validate_on_submit():
        ...
        role = Role(
            name=data["name"],
            auths=",".join(map(lambda v: str(v), data["auths"]))  # 采用高阶函数map来生成一个迭代器，然后用''.join()来序列为一个字符串对象
        )
        db.session.add(role)
        db.session.commit()
        flash("添加角色成功", "ok")
...
def role_edit(id=None):
    ...
    if request.method == "GET":  # 采用get方法对模板中无法直接赋初值的对象进行赋值
        auths = role.auths
        form.auths.data = list(map(lambda v: int(v), auths.split(",")))  # form.auths.data为整形数组，而role.auths为一个可变字符串
    if form.validate_on_submit():
        data = form.data
        role.name = data["name"]
        role.auths = ",".join(map(lambda v: str(v), data["auths"]))  # 采用高阶函数map来生成一个迭代器，然后用''.join()来序列为一个字符串对象
        db.session.add(role)
        db.session.commit()
        flash("编辑角色成功!","ok")
